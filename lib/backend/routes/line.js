  ...
  try {
    ...
    if (text.startsWith("ยกเลิกลิงก์")) {
      ...
      if (parts.length !== 2) {
        await client.replyMessage(event.replyToken, {
          type: "text",
          text: `กรุณาระบุรหัสนักเรียน เช่น "ยกเลิกลิงก์ 164404140090"`,
        });
        return;
      }
      ...
      if (result.affectedRows > 0) {
        await client.replyMessage(event.replyToken, {
          type: "text",
          text: `ยกเลิกลิงก์กับรหัสนักเรียน ${studentId} เรียบร้อยแล้ว`,
        });
      } else {
        await client.replyMessage(event.replyToken, {
          type: "text",
          text: `ไม่พบการลิงก์กับรหัสนักเรียน ${studentId}`,
        });
      }
      return;
    }
    ...
    if (text.toLowerCase() === "ค่าเทอม") {
      ...
      if (linkedStudents.length === 0) {
        await client.replyMessage(event.replyToken, {
          type: "text",
          text: `คุณยังไม่ได้เชื่อมโยงกับนักเรียนคนใด`,
        });
        return;
      }
    ...
    if (existing.length > 0) {
      await client.replyMessage(event.replyToken, {
        type: "text",
        text: `บัญชี LINE นี้ได้เชื่อมโยงกับนักเรียน ${studentName} (${studentId}) ไปแล้ว`,
      });
      return;
    }
