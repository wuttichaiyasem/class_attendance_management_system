...
router.get("/classes", authMiddleware, async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      ...
      WHERE c.teacher_id = ?
      ORDER BY h.class_year, h.group_number
      `,
      [userId]
    );
...
router.get("/:homework_id/students", async (req, res) => {
  ...
  try {
    ...
    const [students] = await conn.execute(
      ...
      FROM students s
      JOIN classes c 
        ON s.homeroom_id = c.homeroom_id
...
router.post("/check-missing", async (req, res) => {
  try {
    ...
    for (const homework of overdueHomework) {
      // 🔹 2. mark missing สำหรับนักเรียนใน class (ผ่าน homeroom)
